...
function probeHosts(target: EventTarget): unknown[] {
    ...
    if (typeof globalThis.document !== 'undefined' && target !== globalThis.document) hosts.push(globalThis.document);
    if (target !== globalThis) hosts.push(globalThis);
...
function testStyleSupport(name: string, styleProperty: string): boolean {
    if (typeof globalThis.document === 'undefined' || typeof globalThis.document.documentElement === 'undefined') return false;

    const style: unknown = globalThis.document.documentElement.style;
...
function testFullscreenSupport(name: string): boolean {
    ...
    const element: unknown = globalThis.document.documentElement;
...
export function resolveEventType(target: EventTarget, type: string): string {
    if (target === globalThis.document && CORDOVA_DOCUMENT_EVENTS.indexOf(type) > -1) return type;
